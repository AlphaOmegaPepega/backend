const Questions = require('../models/StakeholderMng')
...
const createNewquestions = async (req, res) => {
    ...
    if (questions) { //created 
        res.status(201).json({ message: `New Leaderships result created` })
    } else {
...
const updatequestions = asyncHandler(async (req, res) => {
    const {user,Engagement , Leadership,Management,EngagementAnswr,LeadershipAnswr,ManagementAnswr} = req.body
    ...
    question.Engagement =Engagement  ? Engagement  : question.Engagement 
    question.Leadership=Leadership ? Leadership : question.Leadership
    question.Management=Management ? Management : question.Management
    question.EngagementAnswr =EngagementAnswr   ? EngagementAnswr   : question.EngagementAnswr 
    question.LeadershipAnswr =LeadershipAnswr  ? LeadershipAnswr  : question.LeadershipAnswr 
    question.ManagementAnswr =ManagementAnswr  ? ManagementAnswr  : question.ManagementAnswr 
